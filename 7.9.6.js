/**
 * sales_log_v7.9.6.js (Font Color Handling, Clear Non-Delivered on Fix, Cleaned Logs, Add Today Sheet CF Rules, Preserve Trade BG on Non-Delivered, Fix Monthly Col A, Fix Error Clear, Log Sat on Mon, Fix totalErrorsFound scope)
...
function rolloverMonth() {
    withScriptLock(() => {
        ...
        try {
            ...
            archiveSheet.getRange(RANGES.leaderboard).setValues(lbRangeToday.getValues()).setBackgrounds(lbRangeToday.getBackgrounds()).setFontWeights(lbRangeToday.getFontWeights()).setFontSizes(lbRangeToday.getFontSizes()).setFontFamilies(lbRangeToday.getFontFamilies()).setFontColors(lbRangeToday.getFontColors());
