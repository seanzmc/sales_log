...
function summarizeRows(rows) {
  return rows.reduce((summary, row) => {
    ...
    CONFIG.DEAL_SECTIONS.forEach(section => {
      const fiFlag = String(row[section.fiIdx] || "").trim().toUpperCase();
      ...
      if (isDelivered) {
        ...
        const tradeValue = String(row[section.tradeIdx] || "").trim().toUpperCase();
...
function applyMonthlyRowFormatting(sheet, rowsData, startSheetRow, aliasMap) {
  ...
  CONFIG.DEAL_SECTIONS.forEach(section => {
    ...
    for (let i = 0; i < numRows; i++) {
      ...
      const fiFlag = String(rowData[section.fiIdx] || "").trim().toUpperCase();
...
function reapplyCF() {
  try {
    ...
    newRules.push(commonFormatting(SpreadsheetApp.newConditionalFormatRule().whenFormulaSatisfied(`=COUNTIF(INDIRECT("'${CONFIG.SHEETS.DEPOSITS}'!G:G"),$E2)>0`)).setRanges([newCarRange]).build());
    newRules.push(commonFormatting(SpreadsheetApp.newConditionalFormatRule().whenFormulaSatisfied(`=COUNTIF(INDIRECT("'${CONFIG.SHEETS.DEPOSITS}'!G:G"),$L2)>0`)).setRanges([usedCarRange]).build());
