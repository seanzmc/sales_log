/**
 * sales_log_v7.9.4.js (Font Color Handling, Clear Non-Delivered on Fix, Cleaned Logs, Add Today Sheet CF Rules, Preserve Trade BG on Non-Delivered, Fix Monthly Col A, Fix Error Clear, Log Sat on Mon, Fix totalErrorsFound scope)
 ...
 * Includes leaderboard pace rules and duplicate/deposit check rules.
 * This version uses more precise rule identification to prevent duplicates.
 */
function reapplyCF() {
  try {
    const sheets = getSheets();
    ...
    let rulesToKeep = []; // Will hold rules not managed by this script or to be updated

    // --- Define the exact signatures of rules managed by THIS SCRIPT ---
    // Note: Formulas must be EXACT matches to how Apps Script stores them (often without extra spaces).
    // Ranges must also be exact.
    const managedDataRulesSignatures = [
      // New Car Data Rules (Applied to RANGES.todayNewCarDataRange -> B2:G101)
      { formula: "=COUNTIF($E$2:$E$101,$E2)>1", rangeA1: RANGES.todayNewCarDataRange },
      { formula: "=COUNTIF(INDIRECT(\"DEPOSITS!G:G\"),$E2)>0", rangeA1: RANGES.todayNewCarDataRange },
      // Used Car Data Rules (Applied to RANGES.todayUsedCarDataRange -> I2:N101)
      { formula: "=COUNTIF($L$2:$L$101,$L2)>1", rangeA1: RANGES.todayUsedCarDataRange },
      { formula: "=COUNTIF(INDIRECT(\"DEPOSITS!G:G\"),$L2)>0", rangeA1: RANGES.todayUsedCarDataRange }
    ];

    const now = new Date();
    const { daysElapsed, totalDays } = memoizedGetSellingDays(now.getFullYear(), now.getMonth());
    const paceBase = `($Q2/${daysElapsed}*${totalDays})`;

    // Leaderboard Rules (Applied to RANGES.leaderboard -> P2:R25)
    const managedLeaderboardRuleSignatures = totalDays > 0 ? [
      { formula: `=${paceBase}>=10`, rangeA1: RANGES.leaderboard },
      { formula: `=AND(${paceBase}>=8,${paceBase}<10)`, rangeA1: RANGES.leaderboard },
      { formula: `=${paceBase}<8`, rangeA1: RANGES.leaderboard }
    ] : [];

    // --- Iterate through existing rules and decide which ones to keep ---
    existingRules.forEach(rule => {
      ...
      if (bc && bc.getCriteriaType() === SpreadsheetApp.BooleanCriteria.CUSTOM_FORMULA) {
        ...
        // Normalize formula: remove all spaces for robust comparison
        const currentFormulaNormalized = currentFormulaFull.replace(/\s+/g, '');
        const ranges = rule.getRanges();

        // Script-added rules typically apply to a single range.
        // Manually added rules might apply to multiple. We only want to manage single-range script rules.
        if (ranges.length === 1) {
          const currentRangeA1 = ranges[0].getA1Notation();

          // Check against data rules signatures
          for (const sig of managedDataRulesSignatures) {
            // Normalize signature formula for comparison
            if (sig.formula.replace(/\s+/g, '') === currentFormulaNormalized && sig.rangeA1 === currentRangeA1) {
              isManagedByThisScript = true;
              break;
            }
          }
          // Check against leaderboard rules signatures
          if (!isManagedByThisScript) {
            for (const sig of managedLeaderboardRuleSignatures) {
              // Normalize signature formula for comparison
              if (sig.formula.replace(/\s+/g, '') === currentFormulaNormalized && sig.rangeA1 === currentRangeA1) {
                isManagedByThisScript = true;
                break;
      ...
      if (!isManagedByThisScript) {
        rulesToKeep.push(rule.copy().build()); // Keep rules not managed by this script
      }
    });

    let newRules = [...rulesToKeep]; // Start with the rules we're keeping (non-script-managed ones)

    // --- Add/Re-add this script's managed rules ---

    // Add Leaderboard Rules (only if totalDays > 0)
    if (totalDays > 0) {
      const cfLeaderboardRange = todaySheet.getRange(RANGES.leaderboard); // Should be P2:R25
      newRules.push(
        SpreadsheetApp.newConditionalFormatRule().whenFormulaSatisfied(`=${paceBase}>=10`).setBackground("#70ad47").setRanges([cfLeaderboardRange]).build(),
        SpreadsheetApp.newConditionalFormatRule().whenFormulaSatisfied(`=AND(${paceBase}>=8,${paceBase}<10)`).setBackground("#ffee32").setRanges([cfLeaderboardRange]).build(),
        SpreadsheetApp.newConditionalFormatRule().whenFormulaSatisfied(`=${paceBase}<8`).setBackground("#C00000").setRanges([cfLeaderboardRange]).build()
      );
    } else {
      Logger.log("Cannot apply leaderboard CF: Total selling days is zero.");
    }

    // Add Data Validation Rules
    const todayNewCarRange = todaySheet.getRange(RANGES.todayNewCarDataRange); // Should be B2:G101
    const todayUsedCarRange = todaySheet.getRange(RANGES.todayUsedCarDataRange); // Should be I2:N101

    // Rule 1: New Car Duplicate Stock # on TODAY sheet (B2:G101, checks E2 within E2:E101)
    newRules.push(SpreadsheetApp.newConditionalFormatRule()
      .whenFormulaSatisfied("=COUNTIF($E$2:$E$101,$E2)>1") // Formula applies to each cell in range, $E2 adjusts
      .setFontColor(DUPLICATE_STOCK_TEXT_COLOR)
      .setBackground(DUPLICATE_STOCK_FILL_COLOR)
      .setRanges([todayNewCarRange])
      .build());

    // Rule 2: Used Car Duplicate Stock # on TODAY sheet (I2:N101, checks L2 within L2:L101)
    newRules.push(SpreadsheetApp.newConditionalFormatRule()
    ...
    // Rule 3: New Car Stock # found in DEPOSITS (B2:G101, checks E2 against DEPOSITS!G:G)
    newRules.push(SpreadsheetApp.newConditionalFormatRule()
    ...
    // Rule 4: Used Car Stock # found in DEPOSITS (I2:N101, checks L2 against DEPOSITS!G:G)
    newRules.push(SpreadsheetApp.newConditionalFormatRule()
    ...
    setCFRulesSheet(todaySheet, newRules); // Applies the combined list of rules
