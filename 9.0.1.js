...
function tallyCounts(rows, aliasMap) {
    ...
    rows.forEach((row) => {
        CONFIG.DEAL_SECTIONS.forEach(({ fiCol, salesCol }) => {
            const fiFlag = String(row[fiCol - 1] || "")
                .trim()
                .toUpperCase();
    ...
    return rows.reduce(
        (summary, row) => {
            CONFIG.DEAL_SECTIONS.forEach((section) => {
                const fiFlag = String(row[section.fiCol - 1] || "")
                    .trim()
                    .toUpperCase();
                ...
                if (isDelivered) {
                    ...
                    const tradeValue = String(row[section.tradeCol - 1] || "")
                        .trim()
                        .toUpperCase();
...
function ensureMonthlyCF() {
    ...
    CONFIG.DEAL_SECTIONS.forEach((section) => {
        ...
        newRules.push(SpreadsheetApp.newConditionalFormatRule().whenFormulaSatisfied(nonDeliveredFormula).setBackground(CONFIG.COLORS.NON_DELIVERED_DEAL).setRanges([range]).build());
        ...
        newRules.push(SpreadsheetApp.newConditionalFormatRule().whenFormulaSatisfied(salespersonErrorFormula).setBackground(CONFIG.COLORS.SALESPERSON_ERROR).setRanges([salesRange]).build());
    });
...
function reapplyCF() {
    try {
        ...
        newRules.push(
            commonFormatting(SpreadsheetApp.newConditionalFormatRule().whenFormulaSatisfied(`=COUNTIF(${newStockCol}$2:${newStockCol}$101,${newStockCol}2)>1`))
                .setRanges([newCarRange])
                .build()
        );
        newRules.push(
            commonFormatting(SpreadsheetApp.newConditionalFormatRule().whenFormulaSatisfied(`=COUNTIF(${usedStockCol}$2:${usedStockCol}$101,${usedStockCol}2)>1`))
                .setRanges([usedCarRange])
                .build()
        );
        newRules.push(
            commonFormatting(SpreadsheetApp.newConditionalFormatRule().whenFormulaSatisfied(`=COUNTIF(INDIRECT("'${CONFIG.SHEETS.DEPOSITS}'!G:G"),${newStockCol}2)>0`))
                .setRanges([newCarRange])
                .build()
        );
        newRules.push(
            commonFormatting(SpreadsheetApp.newConditionalFormatRule().whenFormulaSatisfied(`=COUNTIF(INDIRECT("'${CONFIG.SHEETS.DEPOSITS}'!G:G"),${usedStockCol}2)>0`))
                .setRanges([usedCarRange])
                .build()
        );
